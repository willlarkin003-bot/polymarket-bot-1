...
| `MIN_AMERICAN_ODDS` | Markets more favored than this are skipped entirely — no edge is worth that little payout (default -200). No corresponding upper reject for underdogs; see the stake variables below for how longshots are handled instead. |
| `MAX_AMERICAN_ODDS` / `EXTREME_AMERICAN_ODDS` / `FAVORITE_MAX_STAKE_USD` / `LONGSHOT_MAX_STAKE_USD` | A bet at or more favored than `MAX_AMERICAN_ODDS` (default +600) stakes the full `FAVORITE_MAX_STAKE_USD` (default $25). Beyond that, the cap tapers down as the bet gets more of a longshot, reaching `LONGSHOT_MAX_STAKE_USD` (default $5) once odds hit `EXTREME_AMERICAN_ODDS` (default +3000) and staying there no matter how extreme it gets — a market implying you'd risk $25 to win $2500 still gets bet, just for a couple bucks instead of $25. Kelly sizing and `MAX_POSITION_PCT` still apply on top — whichever cap is smaller wins. |
