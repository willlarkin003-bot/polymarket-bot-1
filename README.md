...
| `MIN_AMERICAN_ODDS` / `MAX_AMERICAN_ODDS` | Only bet on markets whose implied odds fall in this range (defaults -200 to +600). Filters out both heavy favorites and extreme longshots — e.g. a market implying you'd risk $25 to win $2500 is skipped, same as failing `MIN_EDGE`. |
| `FAVORITE_MAX_STAKE_USD` / `LONGSHOT_MAX_STAKE_USD` | Within that odds range, the stake cap scales with how much of a longshot the bet is (defaults $25 at the -200 favorite edge down to $5 at the +600 longshot edge, interpolated in between). Kelly sizing and `MAX_POSITION_PCT` still apply on top of this — whichever cap is smaller wins. |
